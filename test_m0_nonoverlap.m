...
% 按类型统计占用帧数
type_names = {'空闲', 'trustedfixed', 'priorposknown', 'priortimeknown', 'ordinaryTarget'};
fprintf('\n--- 各频带占用统计 (帧数) ---\n');
fprintf('  %-6s', 'Band');
for ti = 1:5
...
% AP、网格与所有源累积位置可视化
figure('Name', 'M0 场景布局', 'Position', [100 650 700 600]);
...
if ~isempty(M0Logs.TruthLogAll)
    ...
    if any(mask_tr)
        tr_ids = unique(all_ids(mask_tr));
        for ii = 1:numel(tr_ids)
            idx_first = find(all_ids == tr_ids(ii) & mask_tr, 1);
            xy_tr = all_xy(idx_first, :);
            if ii == 1
                h_tr = plot(xy_tr(1), xy_tr(2), 'p', 'Color', [0.2 0.4 1], ...
                    'MarkerSize', 16, 'MarkerFaceColor', [0.2 0.5 1], 'LineWidth', 1.2);
            else
                plot(xy_tr(1), xy_tr(2), 'p', 'Color', [0.2 0.4 1], ...
                    'MarkerSize', 16, 'MarkerFaceColor', [0.2 0.5 1], 'LineWidth', 1.2);
            end
        end
    else
    ...
    % 需要从 TruthLogAll 中按 subtype 区分，但 TruthLogAll 只记录 source_type='prior'
    % 所以用 template_key 前缀区分
    all_keys = {M0Logs.TruthLogAll.template_key};
    mask_pp = strncmp(all_keys, 'prior_pos_', 10);
    h_pp = [];
    if any(mask_pp)
        pp_ids = unique(all_ids(mask_pp));
        for ii = 1:numel(pp_ids)
            idx_first = find(all_ids == pp_ids(ii) & mask_pp, 1);
            xy_pp = all_xy(idx_first, :);
            if ii == 1
                h_pp = plot(xy_pp(1), xy_pp(2), 's', 'Color', [0 0.7 0.3], ...
                    'MarkerSize', 10, 'MarkerFaceColor', [0 0.8 0.4], 'LineWidth', 1.2);
            else
                plot(xy_pp(1), xy_pp(2), 's', 'Color', [0 0.7 0.3], ...
                    'MarkerSize', 10, 'MarkerFaceColor', [0 0.8 0.4], 'LineWidth', 1.2);
            end
        end
    end
    ...
    h_pt = [];
    if any(mask_pt)
        pt_ids = unique(all_ids(mask_pt));
        for ii = 1:numel(pt_ids)
            idx_first = find(all_ids == pt_ids(ii) & mask_pt, 1);
            xy_pt = all_xy(idx_first, :);
            if ii == 1
                h_pt = plot(xy_pt(1), xy_pt(2), 'd', 'Color', [0.9 0.55 0], ...
                    'MarkerSize', 9, 'MarkerFaceColor', [1 0.7 0], 'LineWidth', 1.2);
            else
                plot(xy_pt(1), xy_pt(2), 'd', 'Color', [0.9 0.55 0], ...
                    'MarkerSize', 9, 'MarkerFaceColor', [1 0.7 0], 'LineWidth', 1.2);
            end
        end
    end

    % --- ordinary_target (红黑圆点) ---
    mask_tg = strcmp(all_types, 'ordinary_target');
    h_tg = [];
    if any(mask_tg)
        tg_ids = unique(all_ids(mask_tg));
        for ii = 1:numel(tg_ids)
            idx_first = find(all_ids == tg_ids(ii) & mask_tg, 1);
            xy_tg = all_xy(idx_first, :);
            if ii == 1
                h_tg = plot(xy_tg(1), xy_tg(2), 'o', 'Color', [0.85 0.15 0.15], ...
                    'MarkerSize', 7, 'MarkerFaceColor', [0.1 0.2 0.2], 'LineWidth', 1);
            else
                plot(xy_tg(1), xy_tg(2), 'o', 'Color', [0.85 0.15 0.15], ...
                    'MarkerSize', 7, 'MarkerFaceColor', [0.1 0.2 0.2], 'LineWidth', 1);
            end
        end
    end
...
title('M0 场景布局 — 各类源累积出现位置');
axis equal; grid on;
xlim(Config.area.x_range); ylim(Config.area.y_range);

% 构建图例
leg_h = {}; leg_s = {};
leg_h{end+1} = plot(NaN, NaN, '.', 'Color', [0.85 0.85 0.85], 'MarkerSize', 10); leg_s{end+1} = '有效网格点';
leg_h{end+1} = plot(NaN, NaN, 'r^', 'MarkerSize', 10, 'MarkerFaceColor', 'r');   leg_s{end+1} = 'AP';
